...
(:  let $sections :=
    <multi> {
    parser:passthru-sections( fn:analyze-string($template, $parser:r-sections, 's' ) )
    } </multi>
    :)
...
let $_ := xdmp:log(("~~~~~~~",'SEQ:',xdmp:quote($seq), "ETAGS", xdmp:quote($etagsToBeFixed), 
